...
UCLASS(BlueprintType, Blueprintable, HideCategories = ("Platforms"), Category = "NDI IO",
	   HideCategories = ("Information"), META = (DisplayName = "NDI Media Receiver"))
...
public:
	...
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Settings",  META = (DisplayName = "Metadata",  AllowPrivateAccess = true))
		FString Metadata = FString("");

	const FString GetCurrentMetadata() const;
...
private:
	...
	*/
	void DrawProgressiveVideoFrame(FRHICommandListImmediate& RHICmdList, NDIlib_video_frame_v2_t& Result);
	void DrawInterlacedVideoFrame(FRHICommandListImmediate& RHICmdList, NDIlib_video_frame_v2_t& Result);
